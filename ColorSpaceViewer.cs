...
//{
//    /// <summary>
//    /// نافذة عرض الفضاءات اللونية ثلاثية الأبعاد
//    /// المتطلبات 4 و 5: تمثيل بصري + تفاعل + مزامنة
//    /// </summary>
//    public class ColorSpaceViewer : Form
//    {
//        // ─── عناصر الواجهة ───────────────────────────────────────────
//        private Panel pnlCanvas;       // منطقة الرسم 3D
//        private Panel pnlInfo;         // لوحة معلومات اللون
//        private ComboBox cmbSystem;       // اختيار النظام اللوني
//        private Label lblSystem;
//        private Label lblColorSwatch;  // مربع اللون المختار
...
//        private bool hasDragged = false;   // فرّق بين Click و Drag
...
//        // مدير التحويل اللوني (من الشخص الثاني)
...
//        // ════════════════════════════════════════════════════════════
//        //  بناء الواجهة يدوياً (بدل Designer)
//        // ════════════════════════════════════════════════════════════
...
//            };
//            cmbSystem.Items.AddRange(new string[] { "RGB", "HSV", "Lab (2D)", "YUV (2D)", "YCbCr (2D)" });
...
//                lblColorSwatch, btnPick, lblRGB, lblHSV, lblYUV, lblYCbCr, lblLab
//            });

//            // ── إضافة كل العناصر للـ Form ──────────────────────────
//            this.Controls.AddRange(new Control[] {
//                lblTitle, lblSystem, cmbSystem, btnReset,
//                lblInstruction, pnlCanvas, pnlInfo
//            });

//            // ── ضبط الأحجام عند تغيير حجم النافذة ─────────────────
//            this.Resize += (s, e) => LayoutControls();
//            this.Shown += (s, e) => LayoutControls();
...
//        {
//            int step = 12; // عدد نقاط على كل محور (كبّره = أكثر نقاط = أبطأ)
...
//                        // نرسم فقط نقاط السطح (وجوه المكعب) وليس الداخل
//                        bool onSurface = ri == 0 || ri == step ||
//                                         gi == 0 || gi == step ||
//                                         bi == 0 || bi == step;
//                        if (!onSurface) continue;

//                        // تحويل النقطة: المحاور -0.5 إلى +0.5 (مركز المكعب عند 0)
//                        PointF pt = projection.Project(r - 0.5, gr - 0.5, b - 0.5);
//                        double dep = projection.GetDepth(r - 0.5, gr - 0.5, b - 0.5);

//                        points.Add((pt, Color.FromArgb((int)(r * 255), (int)(gr * 255), (int)(b * 255)), dep));
//                    }
//                }
//            }

//            // رتّب من الأبعد للأقرب (Painter's Algorithm)
...
//        {
//            int hSteps = 36; // عدد زوايا Hue
//            int sSteps = 8;  // عدد مستويات Saturation
//            int vSteps = 8;  // عدد مستويات Value
...
//                double radius = v; // نصف القطر يكبر مع V
...
//                        // تحويل HSV → RGB للحصول على اللون
...
//                        double y = v - 0.5; // المحور العمودي

//                        PointF pt = projection.Project(x * 0.5, y, z * 0.5);
//                        double dep = projection.GetDepth(x * 0.5, y, z * 0.5);
//                        points.Add((pt, col, dep));
//                    }
//                }
//            }
...
//            // ارسم شبكة نقاط Lab في المستوى a*-b* (L* ثابت = 60)
...
//                // Cb و Cr تتراوح 16-240، نحوّلها لـ -112 → +112
...
//        // ════════════════════════════════════════════════════════════
//        //  تحديث لوحة المعلومات (المزامنة بين الأنظمة)
//        // ════════════════════════════════════════════════════════════
...
//            {
//                string line = $"{r.SystemName,-8} {r.Channel1Name}: {r.Channel1:F1}  " +
//                              $"{r.Channel2Name}: {r.Channel2:F1}  {r.Channel3Name}: {r.Channel3:F1}";
//                if (r.SystemName == "HSV")
...
//            pnlCanvas.Invalidate(); // أعد رسم للتحديث
//        }

//        // ════════════════════════════════════════════════════════════
//        //  أحداث الماوس — دوران وتكبير
//        // ════════════════════════════════════════════════════════════
...
//            {
//                isDragging = true;
//                hasDragged = false;   // صفّر عند كل ضغطة جديدة
...
//            // اعتبره drag بس إذا تحرك أكثر من 3 بكسل
...
//        // ════════════════════════════════════════════════════════════
//        //  النقر لاختيار اللون (يبحث عن أقرب نقطة)
//        // ════════════════════════════════════════════════════════════
//        private void PnlCanvas_MouseClick(object sender, MouseEventArgs e)
//        {
//            if (e.Button != MouseButtons.Left) return;
//            if (hasDragged) return;   // كان سحب مش ضغط
...
//            // للـ RGB و HSV: ابحث عن أقرب نقطة على الشاشة
//            Color closest = FindClosestColor(e.Location);
...
//        {
//            double bestDist = 20; // threshold بالبكسل
...
//                        PointF pt = projection.Project(r - 0.5, grn - 0.5, b - 0.5);
//                        double dist = Math.Sqrt(Math.Pow(pt.X - mouse.X, 2) + Math.Pow(pt.Y - mouse.Y, 2));

//                        if (dist < bestDist)
//                        {
//                            bestDist = dist;
//                            bestCol = Color.FromArgb((int)(r * 255), (int)(grn * 255), (int)(b * 255));
//                        }
//                    }
//                }
//            }
...
//        /// <summary>
//        /// استدعِ هذه الدالة من Form الرئيسي لتمرير اللون المختار من الصورة
//        /// </summary>
...
namespace Homwore
{
    public class ColorSpaceViewer : Form
    {
        ...
        // ✅ بناء ذاكرة مؤقتة للنقاط (للنقر السريع)
        ...
        // ✅ معالجة النقر المُصلحة - تدعم HSV بشكل صحيح
        private void PnlCanvas_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button != MouseButtons.Left) return;
        ...
        // ✅ دالة جديدة للبحث عن أقرب لون في HSV
